...
$rawPost = NULL;
$hookSecret = SECRET_ACCESS_TOKEN;
if ($hookSecret !== NULL) {
    ...
    list($some, $hash) = explode('=', $_SERVER['HTTP_X_HUB_SIGNATURE'], 2) + array('', '');
    if (!in_array($some, hash_algos(), TRUE)) {
        throw new \Exception("Hash algorithm '$some' is not supported.");
    }
    $rawPost = file_get_contents('php://input');
    if ($hash !== hash_hmac($some, $rawPost, $hookSecret)) {
...
//		break;
    default:
         ...
         * Log to file
         */
        $file = fopen(date('U'), "w") or die(500);
        fwrite($file, $payload);
        fclose($file);
        die(404);
}